...
## TradeLocker / GATESFX Demo

Your local `.env` file is used for TradeLocker credentials. Keep it private.

The connector defaults to:

- Environment: `https://demo.tradelocker.com`
- Server: `GATESFX`
- API base: `https://demo.tradelocker.com/backend-api`

Dry-run scan:

```powershell
python forex_scalper_bot.py --mode tradelocker-scan --config config.yaml
```

Live TradeLocker execution is intentionally gated behind both the command and config:

```powershell
# First change execution.mode to "live" in config.yaml
python forex_scalper_bot.py --mode tradelocker-live --config config.yaml
```

If TradeLocker/GATESFX returns a different backend route, update only the `tradelocker.endpoints` section in `config.yaml`. If your broker symbols are not plain `EURUSD`, set `tradelocker.symbol_overrides`.
