...
def doctor(cfg: dict[str, Any]) -> None:
    print(f"Python: {sys.executable}")
    print(f"Working directory: {os.getcwd()}")
    print(f"Config loaded: yes")
    print(f"Execution mode: {cfg['execution']['mode']}")
    print(f"TradeLocker env: {os.getenv('TRADELOCKER_ENV', cfg['tradelocker'].get('env', ''))}")
    print(f"TradeLocker server: {os.getenv('TRADELOCKER_SERVER', cfg['tradelocker'].get('server', ''))}")
    print(f"TradeLocker email set: {'yes' if os.getenv('TRADELOCKER_EMAIL') else 'no'}")
    print(f"TradeLocker password set: {'yes' if os.getenv('TRADELOCKER_PASSWORD') else 'no'}")
    for module in ["yaml", "dotenv", "requests", "pandas", "numpy", "yfinance"]:
        try:
            __import__(module)
...
def add_indicators(df: pd.DataFrame, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    bb_mid = out["close"].rolling(s.get("bb_period", 20)).mean()
    bb_width = out["close"].rolling(s.get("bb_period", 20)).std() * s.get("bb_std", 2.0)
    out["bb_mid"] = bb_mid
    out["bb_upper"] = bb_mid + bb_width
    out["bb_lower"] = bb_mid - bb_width
    out["ema_slope"] = out["ema_slow"] - out["ema_slow"].shift(5)
...
def build_signal(pair: str, df: pd.DataFrame, cfg: dict[str, Any]) -> Signal | None:
    ...
    momentum_long = (
        latest["adx"] >= min_adx
        and long_trend
        and latest["macd_hist"] > prev["macd_hist"] > prev2["macd_hist"]
        and 45 <= latest["rsi"] <= 68
        and prev["low"] <= prev["ema_fast"]
        and latest["close"] > prev["high"]
        and latest["body_ratio"] >= 0.45
    )
    momentum_short = (
        latest["adx"] >= min_adx
        and short_trend
        and latest["macd_hist"] < prev["macd_hist"] < prev2["macd_hist"]
        and 32 <= latest["rsi"] <= 55
        and prev["high"] >= prev["ema_fast"]
        and latest["close"] < prev["low"]
        and latest["body_ratio"] >= 0.45
    )
    range_market = latest["adx"] <= float(s.get("range_adx_max", 22))
    range_long = (
        range_market
    ...
    if momentum_long:
        stop = entry - s["stop_atr_multiple"] * atr_now
        target = entry + s["reward_risk"] * (entry - stop)
        side = "buy"
        reason = "Hybrid scalp trend-long: EMA alignment, MACD acceleration, pullback break"
    elif momentum_short:
        stop = entry + s["stop_atr_multiple"] * atr_now
        target = entry - s["reward_risk"] * (stop - entry)
        side = "sell"
        reason = "Hybrid scalp trend-short: EMA alignment, MACD acceleration, pullback break"
    elif range_long:
        stop = entry - s.get("range_stop_atr_multiple", 0.75) * atr_now
        target = min(float(latest["bb_mid"]), entry + s.get("range_reward_risk", 0.9) * (entry - stop))
        side = "buy"
        reason = "Hybrid scalp range-long: lower band rejection with RSI turn"
    elif range_short:
        stop = entry + s.get("range_stop_atr_multiple", 0.75) * atr_now
        target = max(float(latest["bb_mid"]), entry - s.get("range_reward_risk", 0.9) * (stop - entry))
        side = "sell"
        reason = "Hybrid scalp range-short: upper band rejection with RSI turn"
    else:
        return None
...
def tradelocker_market_data(client: Any, cfg: dict[str, Any]) -> dict[str, pd.DataFrame]:
    data = {}
    bars = int(cfg["strategy"]["lookback_bars"])
    resolution = cfg["strategy"]["timeframe"]
    fallback = cfg["tradelocker"].get("market_data_fallback", "none")
    use_broker_candles = bool(cfg["tradelocker"].get("use_broker_candles", False))
    for pair in cfg["universe"]:
        if not use_broker_candles and fallback == "yfinance":
            try:
                data[pair] = download_pair(pair, interval=resolution)
                continue
            except Exception as fallback_exc:
                print(f"Skipping {pair}: yfinance={fallback_exc}")
                continue
        try:
            data[pair] = client.candles(pair, bars=bars, resolution=resolution)
        except Exception as exc:
            if fallback == "yfinance":
                try:
                    print(f"TradeLocker candles unavailable for {pair}; using yfinance fallback.")
                    data[pair] = download_pair(pair, interval=resolution)
                    continue
                except Exception as fallback_exc:
                    print(f"Skipping {pair}: TradeLocker={exc} | yfinance={fallback_exc}")
            else:
                print(f"Skipping {pair}: {exc}")
    if not data:
        raise RuntimeError("No TradeLocker market data available. Check login, account, and symbol mappings.")
    return data


def tradelocker_scan(cfg: dict[str, Any], place_orders: bool = False) -> None:
    from tradelocker_client import TradeLockerClient

    print("Starting TradeLocker scan...")
    client = TradeLockerClient.from_env(cfg)
    print("Logging in to TradeLocker...")
    client.login()
    print(f"TradeLocker connected. account_id={client.account_id} mode={cfg['execution']['mode']}")
    data = tradelocker_market_data(client, cfg)
    signals = scan_data(data, cfg)
    if not signals:
        print("No valid TradeLocker signals right now.")
        return

    live_enabled = place_orders and cfg["execution"]["mode"] == "live"
    for signal in signals:
        print(format_signal(signal))
        if live_enabled:
            print(f"Sending DEMO order to TradeLocker for {signal.pair}...")
            result = client.place_market_order(signal.pair, signal.side, signal.units, signal.stop, signal.target)
            print(result)
        else:
            print("Dry run: order not sent.")


def tradelocker_loop(cfg: dict[str, Any]) -> None:
    from tradelocker_client import TradeLockerClient
    ...
    client.login()
    print(f"TradeLocker connected. account_id={client.account_id} mode={cfg['execution']['mode']}")
    print(f"Continuous scanner running every {poll_seconds} seconds. Press Ctrl+C to stop.")
    sent_signal_keys: set[tuple[str, str, float, float, float]] = set()
    ...
    while True:
        try:
            ...
            signals = scan_data(data, cfg)
            if not signals:
                print(f"{now.isoformat()} no valid TradeLocker signals")
                time.sleep(poll_seconds)
                continue

            live_enabled = cfg["execution"]["mode"] == "live"
            for signal in signals:
                signal_key = (
                    signal.pair,
                    signal.side,
                    ...
                    round(signal.stop, 5),
                    round(signal.target, 5),
                )
                print(format_signal(signal))
                if signal_key in sent_signal_keys:
                    print("Skipping duplicate signal already handled in this run.")
                    continue
                sent_signal_keys.add(signal_key)
                if live_enabled:
                    print(f"Sending DEMO order to TradeLocker for {signal.pair}...")
                    result = client.place_market_order(signal.pair, signal.side, signal.units, signal.stop, signal.target)
                    print(result)
            ...
            print("Stopped continuous scanner.")
            return
        except Exception as exc:
...
def tradelocker_instruments(cfg: dict[str, Any], limit: int = 80) -> None:
    from tradelocker_client import TradeLockerClient

    print("Logging in to TradeLocker...")
    client = TradeLockerClient.from_env(cfg)
    client.login()
    print(f"TradeLocker connected. account_id={client.account_id}")
    instruments = client.instruments()
    print("Configured universe matches:")
    for pair in cfg["universe"]:
        instrument = client.resolve_instrument(pair)
        if instrument:
            symbol = client._instrument_symbol(instrument)
            instrument_id = (
                instrument.get("id")
                or instrument.get("tradableInstrumentId")
                or instrument.get("instrumentId")
                or ""
            )
            print(f"FOUND   {pair}\t{instrument_id}\t{symbol}")
        else:
            print(f"MISSING {pair}")
    print()
    print(f"Fetched {len(instruments)} instruments. Showing first {min(limit, len(instruments))}:")
...
def tradelocker_debug(cfg: dict[str, Any]) -> None:
    from tradelocker_client import TradeLockerClient

    print("Logging in to TradeLocker...")
    client = TradeLockerClient.from_env(cfg)
    client.login()
    print(f"TradeLocker connected. account_id={client.account_id} account_number={client.account_number}")
    print("\nAccounts response:")
    print(json.dumps(client.debug_accounts_response(), indent=2)[:4000])
    print("\nInstruments response:")
    print(json.dumps(client.debug_instruments_response(), indent=2)[:6000])


def tradelocker_test_order(cfg: dict[str, Any]) -> None:
    from tradelocker_client import TradeLockerClient

    if cfg["execution"]["mode"] != "live":
        raise RuntimeError("Set execution.mode to live before sending a TradeLocker test order.")
    test_cfg = cfg.get("test_order", {})
    pair = test_cfg.get("pair", "EURUSD")
    side = test_cfg.get("side", "buy")
    quantity = float(test_cfg.get("quantity", 0.01))
    print(f"Preparing DEMO test order: {side.upper()} {quantity} {pair}")
    client = TradeLockerClient.from_env(cfg)
    client.login()
    print(f"TradeLocker connected. account_id={client.account_id}")
    result = client.place_market_order(pair, side, quantity)
    print("TradeLocker test order response:")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "live-mt5",
            "tradelocker-debug",
            "tradelocker-instruments",
            "tradelocker-scan",
            "tradelocker-loop",
            "tradelocker-live",
            "tradelocker-test-order",
        ],
        default="scan",
    )
    ...
    if args.mode == "doctor":
        ...
        live_mt5(cfg)
    elif args.mode == "tradelocker-debug":
        tradelocker_debug(cfg)
    elif args.mode == "tradelocker-instruments":
        tradelocker_instruments(cfg)
    elif args.mode == "tradelocker-scan":
        tradelocker_scan(cfg, place_orders=False)
    elif args.mode == "tradelocker-loop":
        tradelocker_loop(cfg)
    elif args.mode == "tradelocker-live":
        tradelocker_scan(cfg, place_orders=True)
    elif args.mode == "tradelocker-test-order":
        tradelocker_test_order(cfg)
    return 0
